...
export const useTasksMutators = () => {
  ...
  const {add,update,remove:removeTask, move:moveTask } = useObjectArrayMutators(setTasks);

  const addTask = (addTask: Pick<TaskType, 'text'>) => {
    const now =  new Date();
    add({ id: nanoid(), createAt: now, updateAt: now,checked:false, ...addTask });
  };

  const updateTask = (id: string, updateTask: PartialSetStateAction<TaskType>) => {
